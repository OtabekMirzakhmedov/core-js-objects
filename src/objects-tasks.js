...
function removeProperties(obj, keys) {
  ...
  const newObj = obj;
  // Iterate over the keys to be removed and delete them from the object
...
function compareObjects(obj1, obj2) {
  ...
  // Iterate over keys of obj1 and compare values
  keys1.forEach((key) => {
    // If the value of the key in obj1 is not equal to the value of the same key in obj2, objects are not equal
...
function makeWord(lettersObject) {
  ...
  // Iterate over each letter and its positions
  Object.entries(lettersObject).forEach(([letter, positions]) => {
    // Iterate over each position and assign the letter to the corresponding position in the wordArray
    positions.forEach((position) => {
      wordArray[position] = letter;
    });
  });

  // Join the wordArray to construct the word
